...
void build_result(const char *operation, const char *admin_type,
                  ...
                  char *raw_result, nlohmann::json profile_json,
                  racfu_result_t *result, int raw_result_length,
                  racfu_return_codes_t *return_codes);
...
void racfu(racfu_result_t *result, char *request_json) {
    ...
    if (operation.compare("extract") == 0) {
        ...
        do_extract(request["admin_type"].get<std::string>().c_str(),
                   profile_name, class_name, result, &return_codes);
        // Add/Alter/Delete
    } else {
        if (request.contains("profile_name")) {
            profile_name = request["profile_name"].get<std::string>().c_str();
...
void do_extract(const char *admin_type, const char *profile_name,
    ...
    if (return_codes->racfu_return_code == -1) {
        raw_result =
            extract(profile_name, class_name, function_code, return_codes);
        if (raw_result == NULL) {
    ...
    if (raw_result == NULL) {
        build_result("extract", admin_type, profile_name, class_name, NULL,
                     nullptr, racfu_result, 0, return_codes);
        return;
    }
    ...
    build_result("extract", admin_type, profile_name, class_name, raw_result,
                 profile_json, racfu_result, raw_result_length, return_codes);
...
void do_add_alter_delete(const char *admin_type, const char *profile_name,
    ...
    xml_request_string = generator->build_xml_string(
        full_request_json, running_userid, &irrsmo00_options,
        &result_buffer_size, &debug_mode);

    xml_response_string = call_irrsmo00(
        xml_request_string, running_userid, result_buffer_size,
    ...
    response_json = parser->build_json_string(
        xml_response_string, &(return_codes->racfu_return_code), debug_mode);
    ...
    build_result(operation, admin_type, profile_name, class_name,
                 xml_response_string, response_json, racfu_result,
                 result_buffer_size, return_codes);
...
void build_result(const char *operation, const char *admin_type,
                  const char *profile_name, const char *class_name,
                  char *raw_result, nlohmann::json profile_json,
                  racfu_result_t *racfu_result, int raw_result_length,
                  racfu_return_codes_t *return_codes) {
