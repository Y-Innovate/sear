...
char* XmlGen::build_xml_string(nlohmann::json request, char* userid_buffer,
    ...
    std::string requestOperation, adminType, profileName, className, operation,
        currentSegment, runningUserId = "";
    ...
    for (const auto& item : request.items()) {
        ...
        if (item.key().compare("request_data") == 0) {
            continue;
        }
        // The following options dictate parameters to IRRSMO00 and are not
        // built into XML
        else if (item.key().compare("running_user_id") == 0) {
            runningUserId = item.value().get<std::string>();
        } else if (item.key().compare("result_buffer_size") == 0) {
            *result_buffer_size = item.value().get<uint>();
        } else if (item.key().compare("debug_mode") == 0) {
        ...
        // All other valid header information should be built into the XML
            ...
            // All other attribute information is built into the xml at this
            // level to account for VOLUME/GENERIC/Others
    ...
    if ((request.contains("request_data")) &&
        (!request["request_data"].empty())) {
        build_end_nested_tag();
...
void XmlGen::build_attribute(std::string name, std::string value) {
    // Ex: " operation=set"
...
void XmlGen::build_end_nested_tag() { xml_buffer.append(">"); }
void XmlGen::build_full_close_tag(std::string tag) {
    // Ex: "</base:universal_access>"
    tag = replace_xml_chars(tag);
    xml_buffer.append("</" + tag + ">");
}
void XmlGen::build_close_tag_no_value() { xml_buffer.append("/>"); }
void XmlGen::build_single_trait(std::string tag, std::string operation,
                                std::string value) {
...
void XmlGen::build_request_data(std::string adminType,
                                nlohmann::json requestData) {
    ...
    while (!requestData.empty()) {
        for (auto item = requestData.begin(); item != requestData.end();) {
            ...
            if ((itemSegment.compare(currentSegment) == 0)) {
                ...
                translatedKey =
                    get_racf_key(adminType.c_str(), itemSegment.c_str(),
                                 (itemSegment + ":" + itemTrait).c_str(),
                                 TRAIT_TYPE_ANY, OPERATOR_ANY);
...
std::string XmlGen::convert_operation(std::string requestOperation,
                                      int* irrsmo00_options) {
